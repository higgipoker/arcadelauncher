...
static std::set<std::string> pal_megadrive_games = {
    "gunstar.gen",   "sonic2.gen", "eternalchamps.gen", "sonic3.bin", "rocketknight.bin",
    "comixzone.gen", "ecco.md"};

// ------------------------------------------------------------
// Resources
// ------------------------------------------------------------
Resources::Resources(void) { inited = false; }
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (ent->d_type == DT_REG) {
                ...
                if (rawname != "neogeo") {
                    ...
                    new_game->type = MAME;
                    new_game->exec_command = mame_executable + " " + rawname +
                                             " -inipath " + mame_ini + " -rompath " +
                                             mame_games_folder;
            ...
            if (ent->d_type == DT_REG) {
                ...
                if (rawname != "neogeo") {
                    ...
                    new_game->exec_command =
                        snes_executable + " " + snes_games_folder + "/" + rawname;
...
void Resources::load_genesis_games(void) {
    ...
    std::string genesis_icons_folder =
        Config::data.platform_root + "/genesis/artwork/icons";
    std::string genesis_vids_folder =
        Config::data.platform_root + "/genesis/artwork/vids";
    std::string genesis_executable = Config::data.platform_root + "/genesis/bin/genesis";
    std::string genesis_executable_params = " -f -G 800x600 -r ";
    std::string genesis_executable_params_pal = " -f -P -G 800x600 -r ";
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (ent->d_type == DT_REG) {
                ...
                if (rawname != "neogeo") {
                    ...
                    if (pal_megadrive_games.find(rawname) != pal_megadrive_games.end()) {
                        new_game->exec_command =
                            genesis_executable + genesis_executable_params_pal +
                            genesis_ini + " " + genesis_games_folder + "/" + rawname;
                    } else {
                        new_game->exec_command =
                            genesis_executable + genesis_executable_params + genesis_ini +
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (ent->d_type == DT_REG) {
                ...
                if (rawname != "." && rawname != "..") {
                    ...
                    command =
                        amiga_executable + " " +
                        make_amiga_game(amiga_games_folder, amiga_kicks_folder, rawname);
                ...
                if ((dir_inner = opendir(tmp.c_str())) != NULL) {
                    ...
                    command2 = amiga_executable + " " +
                               make_amiga_game(amiga_games_folder, amiga_kicks_folder,
                                               new_game2->gamename, disks);
...
// make_amiga_game
// ------------------------------------------------------------
std::string Resources::make_amiga_game(const std::string &_games_folder,
...
Icon *Resources::make_icon(const std::string &search_dir, const std::string &name) {
    ...
    if ((dir = opendir(search_dir.c_str())) != NULL) {
        ...
        while ((ent = readdir(dir)) != NULL && !found) {
            if (std::string(ent->d_name) != "." && (std::string)ent->d_name != "..") {
                ...
                if (rawname == filename) {
                    icon =
                        new Icon(SDL::window(), search_dir + "/" + filename,
                                 Config::data.icon_border, Config::data.icon_border_lit);
                    found = true;
                }
            }
        }
    ...
    if (icon == NULL) {
        icon = new GenericIcon(SDL::window(), name, Config::data.generic_icon,
                               Config::data.icon_border, Config::data.icon_border_lit);
    }
...
// get_screen_effect
// ------------------------------------------------------------
ScreenEffect *Resources::get_screen_effect() { return screen_effect; }
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (ent->d_type == DT_DIR) { // found a game dir
                ...
                if (rawname != "." && rawname != "..") {
                    ...
                    new_game->type = PC;
                    new_game->exec_command =
                        Config::data.pc_games + "/" + rawname + "/" + rawname;
                    new_game->icon =
                        make_icon(Config::data.pc_games + "/" + rawname, rawname);
                    new_game->video_name =
                        Config::data.pc_games + "/" + rawname + "/" + rawname + ".avi";
