        ...
        while((ent = readdir(dir)) != NULL) {
            ...
            if(ent->d_type == DT_REG) {
                ...
                if(rawname != "neogeo") {
                    ...
                    new_game->type = MAME;
                    new_game->exec_command = mame_executable + " " + rawname + " -inipath " + mame_ini + " -rompath " + mame_games_folder;
        ...
        while((ent = readdir(dir)) != NULL) {
            ...
            if(ent->d_type == DT_REG) {
                ...
                if(rawname != "neogeo") {
                    ...
                    new_game->gamename = rawname;
                    new_game->type = SNES;
                    new_game->exec_command = snes_executable + " " +
                                             snes_games_folder + "/" + rawname;
        ...
        while((ent = readdir(dir)) != NULL) {
            ...
            if(ent->d_type == DT_REG) {
                ...
                if(rawname != "neogeo") {
                    ...
                    new_game->exec_command = genesis_executable + " -f -G 800x600 -r " + genesis_ini + " " + genesis_games_folder + "/" + rawname;
                    ...
                    new_game->video_name =
                        genesis_vids_folder + "/" + rawname + ".avi";
        ...
        while((ent = readdir(dir)) != NULL) {
            ...
            if(ent->d_type == DT_REG) {
                ...
                if((dir_inner = opendir(tmp.c_str())) != NULL) {
                    ...
                    command2 = amiga_executable + " " + make_amiga_game(amiga_games_folder, amiga_kicks_folder, new_game2->gamename, disks);
...
// make_amiga_game
// ------------------------------------------------------------
std::string Resources::make_amiga_game(const std::string &_games_folder, const std::string &_kicks_folder, const std::string &_game_name, std::vector<std::string> _disks) {

    std::string params;
    int i = 0;

    for(std::vector<std::string>::iterator it = _disks.begin(); it != _disks.end(); ++it) {
...
// make_amiga_game
// ------------------------------------------------------------
std::string Resources::make_amiga_game(const std::string &_games_folder, const std::string &_kicks_folder, const std::string &_disk) {
...
Icon *Resources::make_icon(const std::string &search_dir, const std::string &name) {
    ...
    if((dir = opendir(search_dir.c_str())) != NULL) {
        ...
        while((ent = readdir(dir)) != NULL && !found) {
            if(std::string(ent->d_name) != "." &&
                ...
                if(rawname == filename) {
                    icon = new Icon(SDL::window(), search_dir + "/" + filename, Config::data.icon_border, Config::data.icon_border_lit);
                    found = true;
                }
            }
        }
        ...
        std::string error =
            "could not open target directory: " + Config::data.mame_icons;
        perror(error.c_str());
    }
    ...
    if(icon == NULL) {
        ...
        SDL_Text *t =
            SDL::createText("fonts/atari full.ttf", 12, SDL::window());
        icon = new GenericIcon(
            SDL::window(), name, t, Config::data.generic_icon,
